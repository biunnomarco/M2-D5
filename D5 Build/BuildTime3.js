...
function searchJob(lavoro, dove) {
  let lavoroLow = lavoro.toLowerCase();
  let doveLow = dove.toLowerCase();
  let research = [];
  for (let i = 0; i < jobs.length; i++) {
    if (jobs[i].title.toLowerCase().includes(lavoroLow) && jobs[i].location.toLowerCase().includes(doveLow)) {
      research.push(i);
    }
  }
  let result = [];
  for (let j = 0; j < research.length; j++) {
    delete jobs[research[j]].company_profile;
    delete jobs[research[j]].description;
    delete jobs[research[j]].requirements;
    delete jobs[research[j]].benefits;
    result.push(jobs[research[j]])
  }
  return result;
}
let result = searchJob("", "new york");
console.log("Il numero di annunci corrispondenti è: " + result.length)
console.log(result)